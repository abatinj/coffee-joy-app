...
export default function OrdersPage() {
  ...
  const fetchOrders = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(`*, supplier ( nama_supplier ), detail_transaksi_supplier ( id_stok_bahan_baku, kuantitas, harga_beli_satuan, subtotal )`)
  ...
  return (
    <div className="flex h-screen bg-[#F9F7F4] font-sans">
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        <h1 className="text-2xl font-bold text-black mb-6">Purchase Orders</h1>
            ...
            {orders.map((order) => (
              <div key={order.id_transaksi_supplier} className="bg-white p-6 rounded-xl shadow-sm border hover:shadow-md transition">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-lg font-bold text-black">{order.supplier?.nama_supplier || 'Unknown Supplier'}</h3>
                    <p className="text-sm text-gray-500">{new Date(order.tanggal_transaksi).toLocaleDateString('id-ID')}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-500">Total</p>
                    <p className="text-xl font-bold text-black">Rp {order.total_pembayaran?.toLocaleString() || 0}</p>
                  </div>
                </div>
